...
Limbic_system::Limbic_system()
{
	...
	const float w = 1;
	mPFCneuronGreen = new CtxNeuron(learning_rate_mPFC,learning_rate_mPFC*0.01); //making neurons using neuron template, each has different effect weight
...
void Limbic_system::doStep(bool _reward,
	...
	default:
		mPFC2CoreExploreLeft = 0.1; //returned by getExploreLeft()
		mPFC2CoreExploreRight = 0.1; // getExploreRight()
		if (((float)random()/((float)RAND_MAX))<0.05) {
			exploreState = (ExploreStates)floor((float)random()/(float)RAND_MAX*EXPLORE_NUM_ITEMS); //5% chance of... switching state? 
		}
		break;
	}

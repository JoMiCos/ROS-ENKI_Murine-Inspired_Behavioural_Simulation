...
#include <sys/types.h>
#include <sys/stat.h>
#include "enki_ros_pck/parameters.h" //will it find this?
...
void Robot::react(int step,int collision) {
	...
	if (collision) {
		...
		float xp;
		float yp;
		for(float w=0;w<=width/2;w=w+0.5) { //width = width of robot?
			xp=xCoord+rotx(phi,-w,length/2); //xCoord = Xpos input of Robot::Robot, phi = input angle to Robot::Robot, length = l = input to Robot::Robot
...
**/
	...
	//ros::init(argc, argv, "reward_sub");
	//ros::NodeHandle nh;
	bool reward = ros::Subscriber sub_reward = nh.subscribe("mybot/isRewarded", 1, rewardCallback);
	...
	**/
	...
	for(int i=0;i<2;i++) {
		if (world->getFoodValid(i)) {
			...
			double xf = FOODCOORDX; 
			double yf = FOODCOORDY;
	...
	float placefield1=isPlacefield(0); //isPlacefield(): if index =1 return 1, else return 0.
	...
	if (!(world->getSwapFlag())) {
		...
		if (REWARD_DELAY > 0) {
			if (fabs(placefield1)>0) {
				if (rewardDelayGreen>0) { //what does this do?
		...
		if (rewardDelayBlue==0) {
			world->setRewardVisible(2); //sets reward_visible == 2
...
//	fprintf(stderr,"%f %f\n",Greensw,Bluesw);
	...
	********************************************************/
	...
	if (ioDataCount>0) {
		...
		ioData.phi=phi;
		ioData.dPhi=dPhi;
		ioData.dStep=dStep; //actual data passed to robot?
